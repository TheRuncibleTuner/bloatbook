...
\end{table} 
Just like for boxed scalars, the minimum  size of any object can be obtained by summing the size of the header and the data, and then rounding it up to the nearest multiple of the object alignment.  
 
\callout{callout:minimum-size-estimation-rule}{Estimating Using the Estimating Minimum}{
    Let 
%\begin{verbatim}
    	\textit{header} = the object header size and 
    	\textit{alignment} = the object alignment.
%\end{verbatim} 
The minimum size of an object is $(\sum s_f) + header$, rounded up to the next multiple of \textit{alignment}, where $s_f$ ranges over the primitive sizes of all fields in the object's class and superclasses.  The minimum size of an object is its true size if the JRE packs and rearranges fields to fit into the smallest space.
}
