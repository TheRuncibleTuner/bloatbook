...
Unfortunately, sometimes a \class{BigDecimal} will become
inflated, since certain arithmetic methods will cause a \class{BigDecimal} to
...
Similarly, calling \method{toString()} on a \class{BigDecimal} causes it to
...
There are common data types that are just beyond simple primitives that can take
up a lot of space.  These include character strings, bit sets, dates, and
