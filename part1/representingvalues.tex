...
mimics the functions of the standard integer. 
A \class{BigInteger} almost always requires two objects (a
...
\paragraph{BigDecimal} Like \class{BigInteger}, the Java class
\class{BigDecimal} performs arbitrary-precision
floating point arithmetic, and is immutable.
Primarily, \class{BigDecimal}s are critical for many accounting
and financial applications that involve currency, for two reasons. First, Java's
\code{float} and \code{double} can lose accuracy in computations.
\class{BigDecimal} provides accuracy to an arbitrary number of digits. 
It allows you to control the scale, which is the number of digits to the right
of the decimal place. Second, \class{BigDecimal} provides for a number of
different kinds of rounding behavior, a requirement of many of these
applications.
...
attached to the \class{BigDecimal}.
