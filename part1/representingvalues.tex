...
objects (a \class{BigInteger} plus an \class{int[]}), for a minimum of 48 bytes total.  The
...
\code{float} and \code{double} can lose precision in computations.
...
\begin{table}
  \centering
\begin{tabular}{lrl} \toprule \toprule
Type & Common case & Exceptions \\ \midrule \midrule
...
& & Retains formatted \class{String} on \code{toString()} and some formatting
calls\\
\bottomrule \bottomrule
\end{tabular}
\caption{The cost of \code{BigInteger} and \code{BigDecimal}.}
