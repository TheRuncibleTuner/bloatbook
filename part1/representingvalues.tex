...
parsing, formatting and conversion of dates, have been deprecated and moved to
supporting classes.  

If you should call one of \code{Date's} deprecated methods, such as
\code{getYear()}, \code{Date} will create and retain
...
deprecated, causes this same effect. So beware of using \code{toString()},
explicitly or implicitly, to format dates, as in:

\begin{shortlisting}
	log.println("Timestamp: " + logEvent[i].date);
\end{shortlisting}

The recommended way to access this functionality is through the
\code{DateFormat} classes, for parsing and formatting dates, and using 
\code{Calendar} classes for working in human-oriented units such as years,
months, and hours.  For example:
...
are called \\
\midrule
\code{Calendar} & 428 (424+4) & Not recommended for long-lived storage
of dates\\
\bottomrule \bottomrule
\end{tabular}
