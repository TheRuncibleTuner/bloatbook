...
\class{EnumerationSet}s:
\begin{shortlisting} 
	enum Weekdays = 
	     {MONDAY, TUESDAY, WEDNESDAY, THURSDAY, FRIDAY, SATURDAY, SUNDAY};
    
    EnumerationSet<Weekdays> workdays = new EnumerationSet<Weekdays>();
...
Since an \class{EnumerationSet} is an object, this representation is going to
...
EnumSet is represented by one object, taking 24 bytes. If the Enym type has more
...
for the field and 24 bytes for the \class{EnumerationSet}. This is considerably
more expensive than storing the bit flags as either bits or bytes. 

When possible, sharing can optimize the cost of 
 \class{EnumerationSet}s considerably. Suppose
that there are 1000 employees. The cost of storing the workdays for these
employees using an \class{EnumertionSet} is 28,000 bytes. 
Now suppose 900 employees work Monday
through Friday. These employees can share an \class{EnumerationSet} representing
