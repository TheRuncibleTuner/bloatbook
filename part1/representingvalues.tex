...
a boolean and an enumerated type. In Java, \class{String}s are a very
...
common use cases. \code{String}s are immutable, meant for string data that never
changes once initialized. \code{StringBuffer}s and
\code{StringBuilder}s, on the other hand, can continue to be updated over time.

Using long-lived \class{StringBuffer}s to store stable character strings 
can waste memory\footnote{This discussion
applies equally to \class{StringBuffer} and \class{StringBuilder}. For
simplicity we'll say \class{StringBuffer} to mean either one.}. That's because
\class{StringBuffer}s were designed for building string data over time. They
...
building a \class{String} is fine, but they should only be used as temporaries
in this case.
