...
A comparion of an \class{Employee} object with a
...
\item The size of the \code{char[]} = 16 bytes + 2 * number of characters,
then round up to a multiple of 8
\end{itemize}
This is for the Sun \jre on the 32-bit architecture.}
...
Good software engineering and Java's design
both encourage designs with many small objects.
Since each object in Java comes with a fixed memory overhead, it's easy to
produce data models with very high overhead. Estimating your memory costs
will let you weigh the importance of flexibility and other software
engineering goals when you map your data into classes.

To estimate the size of your data:
\begin{itemize}
  \item  The size of each object is the sum of the object header and the field
  sizes, rounded up to an alignment boundary. These all depend on the
  \jre (see \autoref{chapter:jre-comparison}).
  \item Each time fields are \emph{delegated} to a separate object, it
  introduces new object header and alignment overheads, along with
  space for the pointer. While one level of indirection may not seem like much,
  these overheads add up quickly in a fine-grained design, where there are
  many small objects.
  \item Common field types that require one or more separate objects, like
  \class{String} and \class{Date}, can add a lot of overhead. To estimate the
  cost of a data type, makes sure to include all of the objects directly
...
Since Java gives you so few modeling options compared to a language
like C++, you are often forced to make tradeoffs between one expensive option
and another. The next two chapters are a guide to the common 
...
The decision to delegate functionality to another object sometimes involves making a tradeoff between flexibility and memory cost.
You need to decide how much flexibility is really needed, and you also need to be aware of the actual memory costs.
This chapter provides the basic knowledge for estimating memory costs. 
\begin{itemize}
