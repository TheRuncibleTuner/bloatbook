...
Fields can also be reference fields. Their size depends on the architecture of
the \jre. They are 4 bytes on a 32-bit \jre.
\begin{table}
  \centering
...
%This is not the case for objects with other kinds of overhead, like references.

An exception to this rule is if you are on a JRE that doesn't pack fields
tightly, and you have a lot of primitive fields that carry very little data, such as \code{boolean} or
\code{byte} fields. In that case, the more fields you have, the more overhead.
...
Unlike some other languages such as C++, Java does not let you embed
