...
\paragraph{Modeling in Java} In the spirit of keeping things simple,
Java does not allow you to embed one object in another. 
% to build a single object out of other objects.
You cannot nest an array inside an object, or store objects directly in an
array. You can only point to other objects. Even the basic data type
\class{String} consists of two objects. Single inheritance is the only language feature
that can be used instead of delegation to compose two objects, but single
...
overhead. In contrast, C++ gives you more options for composing data, allowing
embedding, delegating, overlaying with unions, or subclassing with single
