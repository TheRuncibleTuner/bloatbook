...
The same collection class will scale differently in different situations.
...
Collections are pure overhead. Each collection class has fixed
and variable overhead costs, as discussed in Section~\ref{sec:scalability};
These are its cost profile, that determines how it will scale in different
situations.
The fixed overhead is the minimum space needed with or without any elements; the variable overhead is the additional space needed to
