...
You can easily write access and update methods that translate to and from our
...
\begin{figure}
 ...
 % \includegraphics{eight-char-string}
  ...
  offset is always 0 and the length is redundant.}
  \label{fig:substring}
\end{figure}
...
decisions can have a long-lived impact. For these \class{String} optimizations, it's not clear that
there is any performance gain in real-world applications, as opposed to
benchmarks. Since \class{String}s are usually the single largest consumer of
memory .. ..   (It is difficult to test new frameworks in
realistic settings, or to predict how code will be used in the future.) It's
too late and expensive to change the implementation, so all applications must pay the price in memory footprint.
