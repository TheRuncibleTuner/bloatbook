\chapter{Field Patterns for Frugal Objects}
...
supplier pay a delegation cost: an extra pointer and object header, totalling 12
bytes. An interesting question is how much total memory is actually saved? The
...
saved (or wasted) for different fill rates and sizes of delegated fields. Each
...
4-byte pointer cost, only F of them pay for a side object. The average
savings per object is therefore:
\begin{eqnarray}
B-4\ \ -\ \ F(B+8) \nonumber
\end{eqnarray}

Using a side object will result in a positive
savings whenever\footnote{This calculation does not include alignment
costs.
Depending on the other fields in your object, a side object can
introduce new alignment costs.
In a JRE with a header size that is not a multiple of the object
alignment (e.g.the IBM 32-bit JRE), using a side object can
sometimes decrease alignment costs}
:
\begin{eqnarray}
F &<& \frac{B-4}{B+8} \nonumber
\end{eqnarray}
...
always cause the side object to be allocated.
...
exists at every use. If you need concurrent access to the data you have to take
special care to code the checks correctly to avoid race conditions.
...
\begin{shortlisting}
class Product {
    static HashMap<String, String> majorAward = new HashMap<String, String>();
    ..
}
\end{shortlisting}

Even though a \class{HashMap} has its own high overhead, this
design can still come out ahead if there are a small number of major awards.
You can do a similar analysis as you would for side objects to decide if this
approach is worth it (see Section~\ref
for the cost of a hash entry).
A hash entry usually takes more memory than a side object.
Therefore, a side table will start wasting memory
at a lower fill rate when compared to a side object approach.
