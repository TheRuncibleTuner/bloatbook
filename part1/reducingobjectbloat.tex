...
Using a side object will result in a savings whenever the
above is positive, so whenever \footnote{These calculations don't
include alignment costs. A side object can introduce new alignment costs,
depending on the other fields in your object.
If the JRE header size is not a multiple of the alignment (e.g. the IBM
32-bit JRE), a side object can actually reduce alignment costs in some cases.}:
\begin{eqnarray}
F &<& \frac{B-4}{B+8} \nonumber
\end{eqnarray}
} 
