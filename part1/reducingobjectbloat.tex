...
saved is 8 bytes per object on average, or 67\% of the bytes that
were delegated, when the fill rate is 0\%.
When the fill rate is 10\%, only 50\% of the delegated field bytes are saved.
...
that a wrong guess about the fill rate will result in waste, and if
it does, the memory loss will be relatively small.
...
\footnote{This calculation does not include alignment overhead, which may
be increased by delegation, depending on the other fields in your object.
...
approach is worth it (see Section~\ref{sec:}
for the cost of a hash entry).
A hash entry typically takes more memory than a side object.
