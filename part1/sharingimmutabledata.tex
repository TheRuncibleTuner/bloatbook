...
these concerns for the most common data types.
...
Interned \class{String}s are stored in a separate Java heap known as the
\emph{perm space}.
...
There are JVM parameters to adjust the perm space size: \code{-XX:PermSize=128m}
sets perm size to 128 megabytes, and \code{-XX:MaxPermSize=512m} sets the maximum perm size to 512 megabytes. 
Fortunately, the JVM performs garbage collection on the
...
pointer to an \class{Integer} in the pool, provided \code{value} is in range.
