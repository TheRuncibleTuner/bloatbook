...
The complete set of word types is only known at run time, so an \code{enum}
...
\code{equals}. In \autoref{fig:sharing-pool}, \code{A.type == B.type} is
...
duplicates will actually be shared. Using \code{equals} also allows
...
pool for sharing \class{Strings}, implemented by the \jre. To share a
...
Since \class{Strings} are immutable, sharing is safe. However,
...
\emph{perm space}.
Interning \class{Strings} indiscriminately will waste memory, and can
...
fixed limit on interned \code{Strings}.} There are JVM parameters to adjust the perm
space size: \code{-XX:PermSize=128m} sets perm size to 128 megabytes, and \code{-XX:MaxPermSize=512m} sets the
...
concurrent sharing mechanism for \class{Strings}.
...
As of \javafive, the Java library provides sharing pools for \class{Integers}
and some of the other boxed scalars. Unlike the string pool, the \class{Integer}
pool is initialized at class load time to store all \class{Integers} in a fixed range, from -128 to 127 by
...
least as of \javasix. For \class{Boolean} and \class{Byte}, a
shared constant is returned for every possible value. In
general, there is no penalty for always using \code{valueOf}.
