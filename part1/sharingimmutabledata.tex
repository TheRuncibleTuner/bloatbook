...
%this code will create two new \code{String}
%objects for every document. 
it has no way of sharing the
\code{String}, either across documents or across the two fields.
As a result, two new objects are created per document.
...
the code, even across classes. More important, in this example, is that all
documents will share a single \code{String}. A more maintainable
...
A more dynamic version of the problem can occur when data originates from 
...
name-value pairs. The easiest approach is to build a map directly from the
input \class{String}s. Assume in this example that on each
call, \code{getNextString()} returns a new \class{String} with its result:
...
In most applications of this kind, there is a lot of repetition of both property
names and values, across the metadata for all of the documents. 
For data that can be predicted at compile time, we can
use literals to avoid creating so many copies.  One approach is to use \class{String} literals, and upon reading
each property name, check against the set of known property names, and assign it to one of those. 

TODO: Restate this so it's only for a small set of values that are fixed - use
Edith's text. 
TODO: No need to spell out this example in code, since it would be easier to
code using String interning. Describe it briefly/hypothetically in the text,
and then show the code for solving it with enums. Choose property names or
values(for property names that means EnumMap, for values that means Map<String,
Object>, where for some fields Object is an Enum).

\begin{shortlisting}
class DocumentMetadataWithStaticPropertyNames {
	final public static String propertyNames[] = 
		{"format", "comments", "creationDate", ..};
	final static HashMap<String, String> propertyNameMap =
	...
	public String getSharedName(String propertyName) {
		return propertyNames.get(propertyName)
	}

    void handleNextEntry() {
       String propertyName = propertyNames.getSharedName(getNextPropertyName()); 
       String propertyValue = getNextString();
       propertyMap.put(propertyName, propertyValue);
    }
}
\end{shortlisting}
The \class{String}s stored in
\code{propertyMap} are created dynamically.
If there are just a few distinct property names in all of the input pairs, these
...
string interning, Java's built-in way to share \code{String}s.
...
pool for sharing \class{Strings}. To share a
\class{String}, you simply call the method \code{intern} on it, and
everything is taken care of automatically. The \class{String} wrapper object
will be shared, along with its underlying \class{char[]}s. Since
...
There's a memory overhead cost for each shared \class{String}, so
