...
static empty set, in case there are no alternate suppliers. The method
...
If an additional alternate supplier is added, the singleton is replaced by an
\class{ArrayList}. We can choose a
...
uniqueness, and use the \code{contains} method to do so. For \class{HashSet},
\code{contains} would cause a linear search, so instead we rely on \class{HashSet}'s built-in
uniqueness checking. 
...
cases. In our example, \ldots Depending upon how much extra coding you want to
do, you can further optimize by using arrays or direct reference fields, rather than collections, for the small cases.  
