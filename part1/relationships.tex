...
underlying collection\footnote{Note the distinction between an immutable
collection, whose contents will not change, and an unmodifiable
view over an \class{ArrayList} which may still be changing.}. The class \code{Collections} provides static factory
methods for this purpose. Below is the code to create unmodifiable \class{ArrayList}s in the data model:
...
relationship, or to turn on the unmodifiable behavior only during testing.
 ...
 [base it on solution with empty collections lazily allocated, and reflect in
 the diagram and source?]
 [Extra unmodifiable
cost if caller should iterate. ]
