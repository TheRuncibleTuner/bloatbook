...
\centering
 		...
 		\toprule
	 	 Collection & \makebox{with 1 entry} & \makebox{with 4 entries} & \multicolumn{3}{c}{with n entries}\\
...
\centering
 		...
 		HashMap & 144 & 80 & 216 & 184 \\
 	 	...
 	 	\bottomrule
 	 	\end{tabular}
	\caption{Looking at the effect of capacity on the cost of some small collections, comparing
	the default capacity with the minimum to accomodate the number of entries.
...
\centering
 		...
 		\toprule
	 	 ...
	 	 & fixed & variable & comments \\
	 	 \midrule
	 	ArrayList & 36 & 4 & for any n; round total cost up to nearest 8\\
	 	\midrule
...
Returning to our original example, let's suppose that 25\% of
products have no alternate suppliers, 25\% have one alternate supplier, the next 25\% have betwen
two and four suppliers, and the remaining 25\% have more than four. This time
let's also assume that we do need to guarantee uniqueness in the data model.
The code for the \class{Product} class is shown below. As in the case of lazy allocation of
empty collections, we'll need to hide access to the collections behind
...
public class Product {
	...
	private static final int threshhold = 6;
	.. 
		...
		if (numSuppliers == 0) {
			...
			if (numSuppliers <= threshold) {
				...
				if (suppliers.contains(supplier)) {
			 	...
			 	else if (numSuppliers == threshold) {
			 		// Convert to HashSet
			 		suppliers = 
			 			new HashSet<Supplier>(suppliers);
			 	}
			}
			// Add supplier
...
Each collection is initialized to a shared reference to a static empty set,
from the standard library class \class{Collections}. The \class{Collections}
class also provides factory methods which create singleton collections, that 
use much less memory than their more general counterparts. A singleton list and
set are only 16 bytes each. The singleton collections are immutable. In our
example, if we expect there to be deletions, it's easy to write code that simply deletes
the singleton until it's needed again.


Each collection starts as an array, and then once it
grows past a threshhold, it is converted to a \class{HashSet}.
We can choose a
threshhold, say six entries, which triggers a conversion to a \class{HashSet} when the threshhold is exceeded. Here is the class \class{Product}:

The method \code{addAlternateSupplier} adds the supplier to the array if the
size is less than the threshold, otherwise, it adds the supplier to the \class{HashSet}.
...
cases. Depending upon how much extra coding you want to do, you can further optimize 
by using arrays or direct reference fields, rather than collections, for the small cases.  
%This can save even more space.


\section{Specialized Collections}
...
\subsection{Concurrent and synchronized behavior}
...
\centering
 		...
 		\toprule
	 	 ...
	 	 & & & fixed & variable & comments \\
	 	 \midrule
	 	Collections.singleton set &  16 & - & 16 & - &  \\
	 	Collections.singletonList & 16 & - & 16 & - &  \\
	 	Collections.singletonMap &  40 & - & 40 & - &  \\
	 	\midrule
	 	Guava ImmutableSet &  &  & & &  \\	 	
	 	\midrule
	 	UnmodifiableSet &  &  & & &  \\	 	
	...
	\caption{Cost of some specialized collections when they
	contain very few entries and are allocated with default capacity.}
