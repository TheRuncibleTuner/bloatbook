...
savings.
For example, this is the case with singleton collections. In other instances,
...
data model to be able access the list of alternate suppliers. A seemingly simple
...
the E-C diagram for a solution based on using all \class{ArrayList}s,
minimally sized, as we had in \autoref{fig:trimmed-product} in
\autoref{sec:proper-size}.
...
\code{Collections} class. As a result, each of these collections incurs an additional fixed cost,
 the delegation cost of an extra view wrapper. 
The inset in
\autoref{fig:product-unmodifiable-arraylist} shows this in detail.

[Increased from 5.3MB to 6.9MB, an increase of 28\% 
 ]
...
\centering
	...
	\caption{A sampling of special-purpose collections and their costs. When
	applicable, collections are minimally sized.}
