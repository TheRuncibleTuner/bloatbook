...
collections that will never
contain more than a few elements, or that do not grow once the data has been loaded. 
%As a result, there is no performance gain
In these cases, the empty array slots can add up to a significant bloat problem,
The extra space can be eliminated with no loss of performance.
...
growth policy. So if you are wrong and the \class{ArrayList} grows bigger than
$x$, extra capacity will be allocated, which may be worse than just taking the
default. 
 
Another approach is to call the \code{trimToSize} method of \class{ArrayList},
which shrinks the array by eliminating the extra growth space. 
...
Hash-based collections always require some excess capacity to reduce the
likelihood of collisions. The load factor, by default .75, limits the
on the number of elements in the collection relative to the size of the array.
When setting the initial capacity it's important to take the load factor into account,
so that you leave enough headroom. The initial capacity is the number of
buckets. .. finish this ..

Neither \class{ArrayList}, \class{HashMap}, nor \class{HashSet} automatically
reduces the size of its array when elements are removed, or when the collection
is cleared.
