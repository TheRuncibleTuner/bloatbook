...
[Something about a collection appropirate for the scale here, and fixed costs]
...
completed.
In some applications, even at load time the contents of each collection are
known in advance and will remain constant thereafter.  

The Guava \code{ImmutableList} has the same fixed and variable costs as the
standard \code{ArrayList}. So it will provide the
...
a \code{HashSet}'s 200. If you are willing to trade a hash-based element
lookup for a binary search, Guava's \code{ImmutableSortedSet} will save even
more space, with memory costs again the same as \code{ArrayList}.
...
the list of alternate suppliers.  One somehwat naive approach is to expose the
reference to each collection as part of the data model's API, but make
the collection unmodifiable. \autoref{} shows the E-C diagram for a solution
based on using all \class{ArrayList}s, as we had in
...
collection classes.  Rather, they are 
