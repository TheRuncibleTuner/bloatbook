...
The standard library designers chose to implement
\class{HashSet} by delegating its work
to a \class{HashMap}, one with keys but no values.
The \class{HashSet} object is therefore just a wrapper, that points to the
...
Another key decision is \class{HashMap}'s use of a \emph{chaining}
design. The internal structure is an array of hash buckets,
where each bucket is a linked list of \class{HashMap\$Entry} objects, each
...
cost to 136 bytes. That's the minimum memory needed for a \class{HashSet}.
...
\class{ArrayList} can be considered 0, since this comes out of the extra
...
The variable cost of a \class{HashSet} is 24 bytes. Lower variable cost means
that \class{ArrayList} scales much better than \class{HashSet}. So \class{ArrayList} is better both for small and large sets.
 \begin{figure}
  \centering
