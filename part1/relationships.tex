...
leads to memory bloat. A quick look inside the empty collections show that they
are not all that empty! All of the empty collections in the standard library
require at least two objects each. Even if the default initial capacity is
overridden and set to zero, empty collections are still large. A zero-sized \class{ArrayList}
consumes 40 bytes, a \class{HashMap} takes 64 bytes, and the smallest
\class{HashSet} takes 80 bytes.

 \begin{figure}
  \centering
 \includegraphics[width=.80\textwidth]{part1/Figures/collections/inside-empty.pdf}
  \caption{The internal structure of three typical empty collections. Each
  contains at a minimum of two objects.}
...
problem, collections allocate their own internal objects in an eager
fashion. For example, \class{HashMap} allocates its internal array before any
entries are inserted. You might think that eager initialization is not such a
big problem, since entries will be added eventually. However, it's common to
...
\end{figure}
 ...
 may need, allocate them on demand. On-demand allocation requires more checking code, to avoid
 \code{NullPointerException}s.
 
The abstract \class{Collections} class provides a better solution for many
applications. You can initialize collection fields to shared, immutable empty
collections using static methods \code{emptySet()}, \code{emptyList()},
and \code{emptyMap()}\footnote{Static constants EMPTY\_SET, EMPTY\_LIST, and
EMPTY\_MAP provide a similar capability. without the type safety of
generics.}.
In the following example, calling static method \code{Collections.emptyList()}
...
exists at every use. The size method, iterators, and other access functions will
work as in any other collection.
However, you have to be careful not to let any references to these static empty
collections escape their immediate context. If you give out a reference to a
static empty collection, then there is no way to update this reference once an actual
collection is allocated. Instead, you can provide access and
update methods to the relationship, so that the implementation remains hidden.
You will also need to code to interfaces, avoiding concrete classes
except where needed. In this example, \code{Product} declares
a \code{List} of suppliers, so that it can point to the shared empty list or
an \class{ArrayList} once it's populated.
...
right-sizing them, at initialization time or after loading is
completed, can still make a difference.
