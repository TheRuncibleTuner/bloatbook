...
collection that exactly fits your task will result in a space savings. For
example, this is the case with singleton collections. In other instances,
however, collections with specialized functionality, even those that restrict functionality, 
have hidden costs compared to their more
general counterparts. They may be fine at a larger
scale, but do not scale well when there are
many small collections, due to higher fixed costs.
In this section we look at a sampling of special-purpose
collections, and see how they work when they are used to implement
relationships. \autoref{tab:specialized-collections-small} shows
the costs of the special-purpose collections discussed in this chapter.

\subsection{Immutable behavior}
\label{sec:immutable-collections}
The Guava open source framework provides a set of \emph{immutable} collection
...
even be known at load time, and will remain constant thereafter. In
...
[The standard library does not have immutable.  Closest thing is unmodifiable,
though not really the same.] Suppose that we would like to allow users of our
data model to be able access the list of alternate suppliers. A seemingly simple approach is to expose the
...
the E-C diagram for a solution based on using all \class{ArrayList}s, as we had in
\autoref{fig:product-arraylist} in \autoref{sec:choosing-collection}.

In Java, certain behaviors, such as unmodifiable, are not provided directly in
...
\code{Collections} class. As a result, each of these collections incurs an additional fixed cost, the delegation cost of an extra view wrapper. 
The inset in
...
\begin{figure}
 ...
 \caption{A relationship between 100,000 products and alternate suppliers,
 where the alternate \class{Supplier}s associated with each
 \class{Product} are stored in an \class{Unmodifiable ArrayList}.}
