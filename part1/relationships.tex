...
functionality\footnote{We'll use the term \emph{small collections} to
mean \emph{very} small collections,
where the number of elements is (roughly) in the single digits.}.
...
\centering
 		...
 		\toprule
	 	 Collection & \makebox{with 4 entries} & \multicolumn{3}{c}{with
	 	 n entries}\\
	 	 & & Fixed & Variable & Range \\
	 	 \midrule
	 	ArrayList & 80 & 80 & 0 & 0 .. 10 \\
 		LinkedList & 144 & 48 & 24 & for any n \\
 		HashMap & 216 & 120 & 24 & 0 .. 11 \\
 		HashSet & 232 & 136 & 24 & 0 .. 11 \\
	 	\bottomrule
	 	\end{tabular}
	 	
	\caption{Default size in bytes of some common collection classes when they
	contain very few entries. Variable cost is the additional cost per entry above the fixed
	cost. Costs apply only within the specified range.}
...
\centering
 		\begin{tabular}{l||r||rrr}
 		\toprule
	 	 Collection & with 4 entries & \multicolumn{3}{c}{with n entries}\\
	 	 & & Fixed & Variable & Range \\
	 	 \midrule
	 	ArrayList & 56 & 40 & (n/2)*8 & 0 .. 10 \\
 		LinkedList & 144 & 48 & 24 & for any n\\
 		HashMap & 184 & 88 & 24 & 0 .. 5 \\
 		HashSet & 200 & 104 & 24 & 0 .. 5 \\
	 	\bottomrule
 	 	\end{tabular}
	...
	additional cost per entry above the fixed cost. Costs apply only within the
	specified range.}
