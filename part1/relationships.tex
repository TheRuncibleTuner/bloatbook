...
can make a big difference in memory cost.
...
very small \class{HashSets}, each consuming 232 bytes, for a total cost of
22.1MB.
...
\class{ArrayList} with  little extra checking code, and usually without significant performance
 ...
 where the alternate \class{Suppliers} associated with each
...
\footnote{Once a collection grows beyond its initial size, we'll
...
the set, a 16-byte \class{HashMap.KeySet} is created and retained for the
lifetime of the set.}.
...
under the assumption that \class{HashSets} will contain a large number of
...
\centering
	...
	comparing the default capacity with the minimum to accommodate the number of
	entries.
...
\centering
 		...
 		\toprule
	 	 ...
	 	 & fixed & variable & comments \\
	 	 \midrule
	 	ArrayList & 36  & 4 & for any n \footnote{Round total cost up to nearest 8 bytes.} \\
	 	\midrule
	...
	to the minimum to accommodate the number of entries.}
...
the \class{ArrayLists} can be trimmed between these two phases.
...
\begin{figure}
 ...
 \caption{The relationship between \class{Products} and \class{Suppliers} after
 all of the
 \class{ArrayLists} have been trimmed by calling the \code{trimToSize} method.}
  \label{fig:trimmed-product}
\end{figure}
 
\class{HashSets} and \class{HashMaps} do not have \code{trimToSize}
...
\class{Suppliers} are inserted. As the alternate suppliers are populated, many
...
diagram after removing 25,000 empty alternate supplier \class{ArrayLists}.
The diagram now shows only 75,000 alternate supplier \class{ArrayLists},
since there are no more empty \class{ArrayLists}. We therefore adjust the
...
\begin{figure}
 ...
 \caption{The relationship between \class{Products} and \class{Suppliers},
...
\end{figure}
 ...
 access these fields, to avoid \code{NullPointerExceptions}.
...
until a collection is actually allocated. In this example, \code{Product}
...
use \class{ArrayLists} for smaller collections, and \class{HashSets} for larger
collections. 

One catch is that you will not usually know in advance which collections
in the relationship will end up being small and which will grow to be large. Therefore
...
\code{addAlternateSupplier} creates a singleton list to hold the first alternate
supplier, if one is added.
...
If another alternate supplier is added, the code
replaces the singleton list with an \class{ArrayList}. We can choose a
threshold, say six entries, to be the maximum number of alternate
...
to create unmodifiable \class{ArrayLists} in the data model:
...
minimally-sized \class{ArrayLists}. The cost of the collections
...
be multiplied by the number of collection instances. The
memory cost of using synchronized behavior at this scale can quickly add up. For a
...
\centering
	 	...
	 	UnmodifiableMap & - & - & 24 & - & add'l cost \footnote{There is an additional fixed cost
	 	if you iterate over the keys
        or values (not the entries). An unmodifiable view is created over the
        key set or values collection, and will persist for the lifetime of the
        original collection.} \\
	 	\midrule
	 	SynchronizedList & - & - & 24  & - & add'l cost \\	 	
	 	SynchronizedSet & - & - & 16  & - & add'l cost \\	 	
	 	SynchronizedMap & - & - & 32  & - & add'l cost \footnote{There is an additional fixed cost
		if you iterate over the keys,
	    entries, or values. A synchronized view is created over the key set,
        entry set, or values collection, and will persist for the lifetime of
        the original collection.}\\	 	
	 	\midrule
	 	\multicolumn{5}{l}{Guava:} \\
	 	\midrule
	 	ImmutableList & 40 & 56 & 36  & 4 & for any n \footnote{Round total cost up
	 	to nearest 8 bytes}\\
	 	ImmutableSet & 64 & 96 & 64  & 8 & for any n  \footnote{Round total cost up
	 	to nearest 16 bytes}\\
	 	ImmutableSortedSet & 40 & 56 & 36 & 4 & for any n  \footnote{Round total cost
	 	up to nearest 8 bytes}
	 	\\
	 	 \midrule
	 	\multicolumn{5}{l}{fastutil:} \\
	 	\midrule
	 	ObjectArraySet & 32 & 48 & 28 & 4 & for any n  \footnote{Round total cost up
	 	to nearest 8 bytes} \\
	 	\midrule
	 	\multicolumn{5}{l}{Java 1 collections:} \\
	 	\midrule
	 	Vector & 40 & 56 & 36 & 4 & for any n  \footnote{Round total cost up
	 	to nearest 8 bytes} \\ 	
	 	\bottomrule
	 	\end{tabular}
