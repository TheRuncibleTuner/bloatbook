...
standard \code{Arraylist}.  So it won't save any space, but it will provide the
added protection.  If you require set functionality, class \code{ImmutableSet}
costs more than an \code{ArrayList}, but considerably less than a \code{HashSet}.
With 4 elements its total cost is 96 bytes, compared to
the \code{HashSet}'s 200. \code{ImmutableSortedSet} costs even less, the same
as \code{ArrayList}, trading a hash-based element lookup for a binary search.
