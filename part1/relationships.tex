...
TODO: check source code for correctness!
...
A singleton list and set take only 16 bytes each. The singleton collections
are created and initialized via factory methods in \class{Collections}. In our
example, the relationship is first initialized with a shared reference to a
static empty set. If an element is added, a singleton set is created to hold that first element. 
...
\paragraph{\class{ArrayList} and \class{HashSet}}
