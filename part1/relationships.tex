...
Since allocation and copying can be expensive, these arrays are always
allocated with some extra capacity, to avoid paying these growth costs too
often. 
%This is why
By default, the initial capacity of an \class{ArrayList} is 10, and
its capacity increases by 50\% when it is reallocated. 
By default, the capacity of a \class{HashMap} or \class{HashSet}
starts at 16, and grows by a factor of 2 when it becomes 75\% full.

These policies trade space
for time, on the assumption that collections always grow.
However, many applications have hundreds of thousands of
collections that will never contain more than a few elements, or that do not
grow once the data has been loaded. 
%As a result, there is no performance gain
In these cases, the empty array slots can add up to a significant bloat problem.
This extra space can be eliminated with no loss of performance. 
%,unless you take explicit action.

Fortunately, it is possible to right-size collections at creation
time, by specifying an initial capacity.  For example, if you know that an
\class{ArrayList} has a maximum size $x$, which is less than the default size, then it's worth passing $x$ as a
...
be worse than just taking the default initial size. Unfortunately, the standard
collections do not give you much control over the growth policy.
 
Alternatively, you can call the \code{trimToSize} method
which shrinks the array by eliminating the extra growth space. Trimming reallocates and
...
parameters. In hash-based collections, some excess capacity is needed
to reduce the likelihood of collisions, and to limit the expense of rehashing the entries
when the array is reallocated.Extra care is required when setting these parameters, as
mentioned, to avoid increasing the number of collisions, or causing
more rehashing of entries when the collection is growing.
...
\end{figure}
 ...
 The simplest approach is to leave collection fields null
