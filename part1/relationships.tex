...
performance. The \class{HashSet} 
implementation assumes \class{HashSets} will be very large, and sacrifices memory 
space in favor of performance. Additionally, \class{HashSet} is expensive
because it is implemented as a degenerate \class{HashMap}, that is, a
\class{HashMap} with no values, thus reusuing \class{HashMap} code. The internal
structure of a default size \class{HashSet} is shown in
...
The elements of \class{HashSet}s, which include wrappers, arrays and linked
lists of elements, are typical of collections. 

\paragraph{Wrapper Objects.}
Internally, a \class{HashSet} is just a wrapper, delegating all of its work to a \class{HashMap}.
...
\paragraph{Entries.} \class{HashMap} allows callers to iterate over
