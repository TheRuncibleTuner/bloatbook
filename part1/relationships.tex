...
entry. The variable cost of an \emph{entry-based} collection like
\class{HashSet} is typically much higher than that of an \emph{array-based}
collection, such as \class{ArrayList}, that uses array slots to store its entries.
A lower variable cost means that \class{ArrayList} scales much
better than \class{HashSet}.
The variable cost also matters for small collections, when you have a lot of
instances them. Note that
we treat the variable cost of a small \class{ArrayList} to be 0, since we've already counted the 10 initial slots in the fixed cost.
 \begin{figure}
  \centering
...
Looking back over why \class{HashSet} takes more space than \class{ArrayList},
we can see a number of reasons. Some of the extra cost is because of
additional functionality, such as providing uniqueness checking and entry removal in constant time.
...
additional choices for small collections.
Table~\ref{tab:small-collections-default} shows the memory costs of four common classes from
the standard libraries, along with a few
examples from open source frameworks.
The table shows the cost of sample small collections, plus fixed and
variable costs so you can compute the costs of these collections for small sizes
in genearl. All have been allocated with default capacity.
...
LinkedList fixed cost includes sentinel entry, 48 bytes. lower than default AL. 
Variable cost adds up quickly.
With 2 entries total cost is higher than default AL.
...
\centering
 		...
 		HashMap & 144 & 216 & 120 & 24 & for n in 1..11 \\
 		HashSet & 160 & 232 & 136 & 24 & for n in 1..11 \\
...
\centering
 		...
 		\toprule
	 	 ...
	 	 \midrule
	 	ArrayList & 80 & 48 & 80 & 56 \\
 		HashMap & 144 & 88 & 216 & 184 \\
  		HashSet & 160 & 104 & 232 & 200 \\
 	 	\bottomrule
 	 	\end{tabular}
...
\centering
 		...
 		\toprule
	 	 ...
	 	 & fixed & variable & comments \\
	 	 \midrule
	 	ArrayList & 40 & 4 & for any n \\
	 	\midrule
 		HashMap &  64 & 24 & capacity = 2, holds up to 1 \\
 		        &  72 & 24 & capacity = 4, holds up to 2 \\
 		        &  88 & 24 & capacity = 8, holds up to 5 \\
 		\midrule
 		HashSet &  80 & 24 & capacity = 2, holds up to 1\\
 		        &  88 & 24 & capacity = 4, holds up to 2 \\
		        &  104 & 24 & capacity = 8, holds up to 5 \\
	 	\bottomrule
 	 	\end{tabular}
