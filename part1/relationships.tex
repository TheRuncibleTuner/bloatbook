...
access. You use static factory methods from the \code{Collections} class to
create the view.
...
In some cases, the Java 1 collections are available as a less expensive
...
are some minor differences in functionality.  The Java 1 \code{Vector}
class provides for synchronized access, with fixed and variable costs identical to that of the newer \code{ArrayList}.
\class{Hashtable} has similar costs to \class{HashMap}, though a
smaller initial capacity by default.  There is no Java 1 analogue to
\code{HashSet}.
