...
internal objects in an eager fashion. For example, \class{HashMap} allocates its internal array before any
...
even if the default initial capacity is overridden and set to zero, empty collections are still large. A zero-sized \class{ArrayList}
...
\end{figure}
 ...
 require more checking code, however, to avoid \code{NullPointerException}s.
 
For many applications, a better solution is provided by the abstract
\class{Collections} class. You can initialize collection fields to shared,
immutable empty collections, by using the static methods \code{emptySet()},
\code{emptyList()}, and \code{emptyMap()}\footnote{Static
