...
small \code{TreeMap}s, the fixed overhead of the initial \code{TreeMap} --- its
48 byte wrapper object --- is relatively big. This initial fixed cost is quickly
amortized as samples are added.

\callout{callout:fixed-and-per-entry-costs}{Fixed vs
Variable Overhead of Collections}{
...
and \textit{variable}. These combine to determine the cost of the collection
for a given number of entries.

Fixed overhead stays the same, no
matter how many entries are stored in the collection. Small collections with a large fixed
overhead have a high memory bloat factor, but the fixed overhead is amortized away as
...
\code{Samples} object plus the JVM overhead for the arrays. There is no
additional overhead for each entry, since the variable overhead of a scalar
array is 0, and the samples themselves are pure data.
