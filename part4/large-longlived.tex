...
This data abstraction violation is unfortunate and only avoidable be
reintroducing the very wrappers you have sought to avoid. It would be nice if
\code{Edge.add()}, i.e. the implementation of the \class{Set} interface, would
result in a rerouting of all pointers to that edge to instead point to a proper
set. However, in Java, it is not possible to have \code{Edge.add()} to change its
storage. The Smalltalk language\index{Smalltalk} provides a
\code{become} primitive\index{\texttt{become} primitive} that does exactly this,
though at a nontrivial runtime expense: in the worst case, every \code{become}
