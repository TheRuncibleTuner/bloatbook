...
dead ends. First, the difficulty of supporting nodes with widely varying numbers
of parents and children. If all nodes had only a very small number of incident
edges, or a very large number, then specialized implementations are possible.
Second, optimizing storage has come at the expense of extensibility; one can
remove the use of \class{Edge} objects, but, to support edge labels seems to
require

any highly optimized implementation does not lend itself to adding edge
labels.
