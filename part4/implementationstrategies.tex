...
\section{The Single Strong Owner Principle}

Many lifetime management bugs arise from shared ownership, that is, when an
object that is simultaneously part of more than one data structure. When an
object is shared, as introduced in \autoref{sec:shared-ownership}, it
becomes hard to keep track of what actions need to be taken in order to make
that object reclaimable by the \jre. Even if you make your best effort to avoid
this problem, such as by using weak references, you can still have problems. The
diamond structures described in \autoref{sec:strongweakdiamonds} are a good
example of a case where, even with weak references, objects may stick around too
long. What programming patterns can help to avoid these problems, so that you
aren't left hunting down hard to diagnose memory leaks late in the development
lifecycle?
...
A more practical spin on this single-owner ideal is that every object should have
a \emph{home base}.

\begin{shortlisting}
class SingleStrongOwner {   
...
On its own, the \class{SingleStrongOwner} class provides a repository for strong
...
\class{SingleStrongOwner.own()} on every object that you create. This would
...
you can't simply associate one \class{SingleStrongOwner} repository with a
...
structure:
\begin{shortlisting}
class SingleStrongOwnerFactory {
   public SingleStrongOwner newOwner() {
      return new SingleStrongOwner();
   }

   public Map newMap(SingleStrongOwner sso) {
      return new WeakHashMap() {
         public Object put(Object key, Object value) {
            sso.own(key); return super.put(key, value);
         }
      }
   }
}
...
\begin{shortlisting}
class ThreadLocal_SingleStrongOwnerFactory extends SingleStrongOwnerFactory {
   ThreadLocal<SingleStrongOwner> threadLocals = new ThreadLocal<SingleStrongOwner>();
   
   protected void own(Object o) {
      // the thread's SingleStrongOwner assumes ownership
      return threadLocals.get().own();
   }

   public SingleStrongOwner newOwner() {
      SingleStrongOwner sso = new SingleStrongOwner();
      threadLocals.set(sso);
      return sso;
      // the caller will now have the only strong reference to the SingleStrongOwner repository, we maintain only a weak reference to it
   }
}
\end{shortlisting}

But this implementation suffers from memory drag. After the thread's task
completes, the \tls maintains a reference to the \class{SingleStrongOwner} and
...
task begins, you can grab a strong reference to the \class{SingleStrongOwner}
repository, and have the \tls maintain only a weak reference to
it:
\begin{shortlisting}
class ThreadLocal_SingleStrongOwnerFactory extends SingleStrongOwnerFactory {
   ThreadLocal<WeakReference<SingleStrongOwner>> threadLocals = new ThreadLocal<WeakReference<SingleStrongOwner>>();
   ...
   public SingleStrongOwner newOwner() {
      SingleStrongOwner sso = new SingleStrongOwner();
      threadLocals.set(new WeakReference(sso));
      return sso;
      // the caller has the only strong reference to the SingleStrongOwner repository, we maintain only a weak reference to it
   }
}
\end{shortlisting}
...
\begin{shortlisting}
SingleStrongOwnerFactory requestLocals = SingleStrongOwnerFactory.newThreadLocalFactory();
void doLogin() {
   SingleStrongOwner myLocals = requestLocals.newOwner();
   Object obj = new Object();
   restOfWork(obj);
...
it so that the \class{SingleStrongOwnerFactory} is a local variable of the
