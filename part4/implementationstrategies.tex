...
\subsection{Avoid Weak/Soft-Strong Reference Cycles}

\subsection{The Single Strong Owner Principle}
...
thread. If so, you can leverage the thread local storage mechanism to implement
...
completes, the thread local storage maintains a reference to the
\class{SingleStrongOwner} and all the owned objects. It will only be overwritten
either when the same thread is scheduled to process a new task, or when the
thread terminates. You could overcome this by adding a \code{clear} method, and
inserting a call to it at the right place in your code:
\begin{shortlisting}
public void clear() {
...
repository, and have the thread local storage maintain only a weak reference to
it:
\begin{shortlisting}
class ThreadLocal_SingleStrongOwnerFactory extends SingleStrongOwnerFactory {
...
%simply won't work, because it relies on thread-local storage. Support for
