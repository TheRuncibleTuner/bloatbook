...
to another class increases memory cost. There is a trade-off
between memory cost and flexibility, and you have to decide whether the
flexibility gained from delegation is really worth the cost. However, when
fields are used rarely, delegating them to a side class can actually save
memory, since a side object only has to be allocated if and when it is needed. 
In this case, there is no trade-off, since delegation is always the right
decision.
...
are never used, so 12 bytes in a \class{Product} object are wasted. If you want
to be able to cache as many products as possible in memory, then wasting 12 bytes per
product is significant. To save memory, all of the alternate supplier information
