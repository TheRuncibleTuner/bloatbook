...
Oops! The variable \code{v} references the value, not the key. Therefore, this
...
You can do so in a new \class{CacheSoftReference} wrapper class:\footnote{It
would be nice if the \class{ConcurrentHashMap} implementation let you extend its
implementation so that its \class{\$Entry} class extended soft reference; the
\class{\$Entry} would serve this role perfectly.}
...
loop indefinitely, and call \code{q.remove()} rather than \code{poll()};
