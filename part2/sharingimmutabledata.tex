...
\section{String Literals and Sharing}
...
\end{shortlisting}
Even when duplicate string literals are loaded
from different classes and different packages, the JVM stores just a single
copy.

However, when a new string is created during execution, it is stored in the
heap by default, and it is possible to end up with a lot of string duplication.
In fact, duplicate string data is a leading cause of bloat. One way to avoid
string duplication is to convert dynamic strings to static strings, if
possible. As an example, suppose you are reading in property name-value pairs from 
files into tables:
\begin{shortlisting}
...
there are only 16 distinct property names in all of the input pairs, these
property names will be duplicated many times in the heap.
However, if you know in advance that there are only 16 properties, then you can
use an enumerated type, so that the strings are represented as integers at
runtime. 
\begin{shortlisting}
enum PropertyName = {...};
...
A common
error is to create a new string when you don't have to. For example, in the
following code, there is no need to create new strings from the literals.
\begin{shortlisting}
class BunchOfStringCopies {
	String name1 = new String{``Harry''};
	String name2 = new String{``Tom''};
	String name3 = new String{``Harry''};
}
\end{shortlisting}
Even if the standard library is smart enough to share the character arrays, this
code still creates redundant \code{String} objects.
...
Be careful about new String();

It is possible to do even better, if you have the luxury of modifying both ends of 
the communication channel, i.e. both the serialization and this deserialization code. 
There are only 16 distinct names used in all instances of this configuration map. 
This seems like a perfect case for an enumerated type. An enumerated type can be used to represent 
strings as numbers at runtime. The only place the strings are stored is in the string constant pool. 
Each class, when compiled, keeps a pool of the strings that are used by code in that class. 
In this way, an enumerated type is an even more highly optimized sharing pool than that provided by 
the interning mechanism:
enum PropertyName = {...}; void handleNextEntry() {
PropertyName key = getNextPropertyName(); Object value = getNextString().intern(); map.put(key, value);
}
example from Nick.
