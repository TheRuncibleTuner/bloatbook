...
%%  TODO 20110525 different cases: pause time, throughput, startup time.. JRE
...
reclaimed. It is governed by a number of configuration choices that you can
make. These configuration choices guide the schedule that the collector follows.
This includes, for example, the frequency of collection and the lengths of
pauses your application experiences.
\autoref{chapter:advanced-lifetime-techniques} goes into more detail on the
tuning parameters at your disposal. In any case, the collector will obey some
basic principles, dictated by how your data structures are interconnected, when
determining \emph{what} to collect.

% \paragraph{What a Collection Collects: Reachability and Unique Ownership}
Each time a garbage collection occurs, the collector scans the heap for objects
that might possibly be used in the future.
...
themselves, and the edges are field or array slots that result in a pointer from
one object to another. \index{Heap, as a graph of objects} Recursively, an
object is live either if it is a referenced either by a live object, or, in the
base case, by a \emph{root}. The roots of garbage collection % those that come
from outside the Java space include: objects serving as monitors, objects on the
stack of a method invocation in progress, and references from native code via
the Java Native Interface (JNI). Every other object not live, in this sense, is
...
nodes from the former; note how this property is reflexive, in every node at
least dominates itself. This property is important for determining which other
objects will be garbage collectible when an object is reclaimed: those it
dominates. If an object only dominates itself, but isn't a root of the graph,
then you know that it has \emph{shared ownership}. Otherwise, it is
...
% TODO 20110525
% three cases: drag forever, drag until next iteration, and drag because GC
% never runs
...
% TODO 20110525 each reference has a certain lifetime scope
% an object is kept alive depending on which assortment of references reach it,
% are on paths from GC roots to that object
% TODO 20110525 put instance field first
...
\centering
\fcolorbox{black}{Light}{\begin{minipage}{0.49\textwidth}
...
The first three are cases of unique ownership of an object. There exists
...
\code{B} will become garbage collectible only under two circumstances. Once
\code{A} becomes collectible, then so will \code{B}. Notice how, in this way, a
...
%\code{B} garbage collectible before \code{A} is to break all paths of
...
static fields all classes loaded into that class loader as garbage collectible.
...
%be garbage collectible if the indicated dominating reference is set to
...
% TODO 20110525 maybe a table of references:
% kind             purpose      sections where discussed
% weak             correlated
% soft
% final/phantom
% thread-local
...
% TODO 20110525 mention weak hashmap
% guava has more cases
...
part, these days, the \class{FinalReference} objects are allocated when the
...
it feels like it.\index{References, Phantom}
