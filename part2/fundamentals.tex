...
Unlike in a language such as C, you cannot dictate that the stack contain a
structure, it can only contain pointers to structures that are stored on the
heap. In C, when you write:
\begin{shortlisting}
struct {
   int x;
} g;
...
g.x = 3;
\end{shortlisting}
the stack frame for this method will have one extra 32-bit slots, used to store
the integer field {\tt x} of the local variable {\tt s}. 
...
is called \emph{stack allocation} of objects\index{Stack Allocation}, and is
...
g.x = 3;
\end{shortlisting}
Unfortunately, there is no easy way to determine specifically which of your
calls to {\tt new} will be, or were in any particular run, stack allocated by
the JIT. If you are curious, the best you can do is to analyze performance both
with and without the underlying anlaysis. On Sun JVMs, you can disable the
analysis by adding {\tt -XX:-DoEscapeAnalysis} to your application's command
line.

\paragraph{The Heap}
\index{Heap}
Otherwise, each call to {\tt new} usually results in an allocation on one of
several heaps. \autoref{fig:heaps_and_stacks_java} and
...
\end{subfloat}
	...
	purposes.]{\label{fig:heaps_and_stacks_java}\includegraphics[width=0.575\textwidth]{part2/Figures/lifetime/heaps_and_stacks_java}}
