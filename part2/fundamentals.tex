...
\chapter{Lifetime Management}
\label{chapter:lifetime-fundamentals}

[NOTE: Section 12.1 has way too much detail.  I think the whole section could be
deleted, but if you want to keep some of the points, should be much shorter,
maybe one page.  It's certainly out of place after Chapter 11.  The remaining
sections are more appropriate, especially Basic Ways of Keeping an Object Alive
-- I like that --- Edith]
...
\oracle \javasix Update 21 and \ibm \javasix Service Release 2. When this
...
underlying anlaysis. On \oracle JVMs, you can disable the analysis by adding {\tt
...
the general native heap. The \oracle \jre is in the former camp, while the \ibm and
JRockit \jres are in the latter camp. The \oracle \jres call this heap the
\emph{permspace} heap. This name came about because the heap contains objects
and other data that are, at least for the most part, immortal. On \oracle \jres, you
...
%specified on the command line. In some cases, you may find that your
%application requires a suspciously large maximum size for Permspace.
...
to pause the threads in order to perform reclamation.\index{Concurrent Garbage
Collection}} It requests that the threads stop, and then waits until each thread
...
\oracle JVMs, you will sometimes see a variant of this: \code{GC overhead limit
...
nicknamed \emph{GC death}. In this situation, the \jre is
...
garbage collection. If you are using a \oracle JDK build of the \jre, then you can
use the \code{jstat} tool\index{\oracle jstat tool}.\footnote{The \code{jstat} tool
...
[(GSS) These next 3 section headers are for the table of contents to show what
we are likely to have in a final version.  The placement of these headers
isn't right relative to the rest of the section. These are just section names
that I'd like to stand out in the TOC for the reviewers to see. They're
common cases, and we reference some of them in the first half of the
book.]
\subsection{Example.  Correlated Lifetime: Sharing Pools}
\label{sec:advanced-lifetime-sharing-pools}
\subsection{Example.  Correlated Lifetime: Annotations}
\label{sec:advanced-lifetime-annotations}
\subsection{Example.  Time-space Tradeoffs: Caches}
\label{sec:advanced-lifetime-caches}
% Sometimes, you need an object to stay
% alive, or not, in a way that has nothing to do with whether it is reachable from local variables or static fields. For
...
make a partial effort to clip soft references in a least-recently used (LRU)
...
basis for cache implementations. In reality, no contemporary \jre uses a true
LRU policy for clipping soft references. For example, with \oracle \jres, it is
possible that all soft references that haven't been used in 6 minutes will be
automatically cleared, even though the heap has 500MB of space free, out of a
600MB heap. \autoref{sec:soft-reference-details} has more details on this
subject. Soft references are still useful, despite this limitation, but you will
...
object is directly referenced ownly by strong references, but that it is
...
For example, \class{WeakHashMap} does a good job of completely hiding reference
...
\callout{reference-queue-rule}{Reference Queue Polling Rule}{
A reference queue must be polled at least as often as \class{Reference} objects
are created, that are destined for that queue. Otherwise the queue will leak
memory, growing unboundedly.}

Second, since you need to poll wherever a queueable item is created, you may run
into a severe degree of lock contention. The \code{poll} method of
\class{ReferenceQueue} does synchronize access to the queue, so you don't need
...
different machine. For example, a Java \class{DatabaseConnection} object has
...
around this problem.\index{Finalization}\index{References, Phantom} With the
...
performance due to contention of threads trying to acquire those locks. The
threads need to sleep or spin-lock before they can enter the contended
critical section.\footnote{Spin-locking, optimistically assuming that the lock
will be available in the near future, retries the lock acquisition a few times
in a tight loop. This can help, in the case of limited lock contention, but can
result in wasted CPU cycles in the case of severe contention.}

Java provides an alternative way to implement a permanently resident pattern
called \emph{\tls\/}. \Tls provides a easy way to store cloned data, so that
each thread has its own copy. It is impossible for one thread to access another
thread's local storage. When you store objects in a thread's local storage, the
objects will live for as long as the thread does, unless you explicitly
\code{null} them out or otherwise otherwrite the entries first.
...
for it: \code{ThreadLocal<DateFormat>}.
