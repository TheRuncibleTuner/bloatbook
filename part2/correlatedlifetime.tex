...
%these features correctly can be difficult. There are ``gotchas'', surprises and
%tricky intricasies, that you will need to navigate. The standard library, and
%some third party libraries as well, help with using these features correctly.




\paragraph{Using Weak References}

As introduced in \autoref{softweak}, Java offers a feature called \emph{weak
references}. 

Compared to normal (strong) references, such as  instance and
static fields, and local variables, 
a weak reference provides an alternate lifetime
implications

: . A weak
reference is a is manifested is a wrapper object: to weakly reference an object
\code{obj}, you create an instance \code{new WeakReference(obj)}. For the most
part, this weak reference is like any other reference, in the sense that you can
traverse it to access other objects. The only difference is that it does not
inhibit \code{obj} from being reclaimed.
...
call its \code{get} method. If this method returns \code{null}, then you know
