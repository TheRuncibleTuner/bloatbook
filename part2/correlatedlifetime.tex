...
\begin{wrapfigure}[12]{l}{0.465\textwidth}
\centering
\begin{framedlisting}
...
\index{References, Phantom}
\index{Finalization}

%Any Java objects uniquely owned by this object will be reclaimed in the normal
%course of garbage collection. It is those resources that are \emph{implicitly}
%tied to a Java object, such as file descriptors, socket connections, and
%database resources such as compiled queries, that require special attention.
...
Finalization is the easier of the two to use, but the most fraught with danger.
If you implement a \code{finalize} method in a class, then every instance of
...
the \code{finalize} method of the enqueued objects.
...
% Second, phantom references give you the option of having more than one cleanup
% queue and thread, in contrast to finalization where there is a single
% finalizer queue and (usually) a single finalizer thread.




%Behind the scenes, the \jre allocates an extra object (of type
...
% Even if it did, it also does not know anything about the relationship between
% Java objects and those resources.
...
The Java language specification provides no assurances of how often, or even
...
terminates, by calling \code{System.runFinalizersOnExit(true)}. This is a
deprecated part of the \class{System} API. However, it is hard for the \jre to
...
%management strategy. If you can establish a correlated lifetime pattern, such
%as that the external storage should be reclaimed when an event occurs, or when
%a method returns, then you should do so.

% As long as the resources your application uses fall within the \jress
% boundaries, then the automated mechanisms, such as garbage collection and
% class loading, work without much intervention on your part. For the more
% complex cases involving external resources, such as a database connection, the
% nicely automated world starts to fall apart a bit.
...
instance of \code{font} is not used beyond the execution of that iteration, it
seems like one could run this program on a small heap, for any value of
\code{N}. 
%The heap need only fit what it takes to allocate the handful of
%objects that represent an instance of \code{java.awt.Font}. Try it out, for
%various values of \code{N} and of
%\code{-Xmx}. On \oracle and \ibm \javasix \jres, 
Instead, you will find that, 
as \code{N}
increases, you will very likely need to increase the maximum heap size allotted
to the \jre. If you don't, this code will fail with a \class{java.lang.OutOfMemoryError}.
...
Therefore, as the loop iterates, instances of \class{Font} begin to pile up the
finalization queue.
% Periodically, the background finalizer thread wakes up and
%cleans up the backlog. Unfortunately, if this loop nest creates many
%\class{Font}s quickly, or there are a large number of threads doing so
%concurrently, the finalizer background thread becomes overwhelmed. 
You can put
your application's threads to sleep in order to let the finalizer thread catch
up. Try sticking in this code: \code{if (i \% 100000 == 0)
System.runFinalization()}. This change works around the problem.
% \end{comment}
...
%\code{finalize} method. This can lead to dragging of the resources, exactly
%like the problem of memory drag described earlier. Second, using finalization
...
\paragraph{An Alternative: Explicit Dispose/Close}
...
reference with a reference queue. Both have a \code{get} method. When you 
invoke this method on a weak reference, you get back the referant object, or
\code{null} if the \jre has clipped the reference. 

In contrast, the \code{get} method of a phantom reference always returns
\code{null}. This seems strange, but is a necessary part of ensuring that the
...
information about the referant object, but the \code{get} method does not give
you anything. One way around this problem is to implement a subclass of
\class{PhantomReference}  that stores that necessary information. The only
...
\class{PhantomReference} that strongly referenced the referant, then the
...
You may be tempted to use phantom references to help manage pools. For example,
when pooling connections to a remote data source, you will probably implement
facades around the connection instances. A call to \code{close} on a raw
connection instance actually closes the underlying connection. A call to
\code{close} on a connection facade merely returns it to the pool, making it
available for the next use. It is indeed tempting to try to avoid the need for
an explicit call to \code{close}. After all, the connection facade object should
...
call to \code{close}. This sounds nice.

Unfortunately, this design is prone to livelock failures.\index{Livelock} The
...
warn you when the proper \code{close} method was not invoked. If you do only use
...
\begin{itemize}
  ...
  and instead use the built-in \class{WeakHashMap} or similar functionality from
  other libraries, such as the Guava \code{MapMaker}.
...
% this was going to be a table with key versus value reference type, and the use
% case
%\begin{table}
%\centering
%\begin{tabular}{cc}
