...
internal object headers, is the overhead of the representation of the data. Poor designs waste
more space on these overheads, and thus have a high bloat factor. \autoref{chapter:memory-health} also introduces a diagramming technique,
...
The diagram separates out the two kinds of memory consumers:
...
the application's \emph{entities}, such as your
...
and objects, and the construction of entities from multiple objects.
...
\autoref{chapter:sharing-immutable-data}, \nameref{chapter:sharing-immutable-data}, provides techniques for
...
\ref{chapter:dynamic-records} are about using
off-the-shelf collection classes. \autoref{chapter:brief-introduction-collections} is
an overview of collection resources and costs.
%in the standard library and open source frameworks
The next three chapters go through memory usage patterns for the different ways
...
lifetime requirements of your data, mapping your needs to one of a number of common lifetime patterns. Many
problems can be avoided by carefully understanding the reasons for retaining data in memory.
You may need some objects to remain alive only in conjunction with related objects or events,
while some other objects hang around to avoid the performance cost of recomputing them.
\autoref{chapter:lifetime-fundamentals} provides a primer on the memory
management facilities that Java offers, to help you implement the lifetime policies of your data.
...
\autoref{part:3} covers issues related to the scalability of your designs.
\autoref{chapter:estimating-scalability} teaches you strategies for estimating
the scalability of a design as load increases, using the
Entity-Collection Diagram and Bloat Factor that were introduced in \autoref{chapter:memory-health}. 
It will help you determine the relationship between load and memory usage, and assess whether your
design will fit into memory.
\autoref{chapter:large-long-lived} covers cases when your data model does not fit, even after
applying the tuning methodology of \autoref{part:1}. This chapter gives you techniques for 
achieving greater scalability by stepping outside of object orientation, 
while remaining within the
confines of Java, and without resorting to the use of secondary storage. 

The memory cost estimates in the book are mostly from the 32-bit \oracle \javasix \jre.
\autoref{chapter:jre-comparison} is a reference for estimating memory costs, giving more information for the
most common \jres and architectures. \autoref{chapter:jre-options} covers memory configuration
options for these same \jres.
