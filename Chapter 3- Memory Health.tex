...
When you are designing a data structure to accomplish a task, you design classes to represent application entities, and choose collections in which to store them. Small choices you make in both cases can make a big difference in memory costs. For example, the choice of the initial capacity of a collection can make or break the scalability of your design. The memory health approach looks at entities and collections separately, since there are different kinds of choices and pitfalls in each.
...
\callout{The Entity-Collection (EC) Diagram}{To better understand the
  ...
  Think of an EC diagram as a modified form of an Entity-Relation (ER)
  diagram, where you summarize all implementation details, like the
  attributes of your entities, in a single number: the space occupied
  by those attributes.

  Figure~\ref{fig:content-schematic} shows the basic elements
  of an EC Diagram.
% This diagram summarizes the cost breakdown of a data structure.

%\begin{figure}
%  \centering
\begin{center}
  \includegraphics[width=0.15\textwidth]{Figures/content-schematic}
\end{center}
...
schematic of entities and relationships. Unlike UML diagrams,
relationships are elevated to be entities, rather than edges. The
reason is that in a language like Java, implementation of the
relationship itself is often a big cause of expense, so it is
important to make that visible. A very common pattern is to implement
relationships as collections.

\indent There are two types of boxes, or entities, in a content schematic. Octagons represent collection infrastructure, and rectangles represent all other entities. An edge from a collection to a rectangular entity indicates membership of the entity in the collection. The notation $xN$ inside each entity means there are $N$ instances of the entity. The expression \emph{xN = M} bytes means that there are \emph{N} instances occupying \emph{M} bytes of memory. An edge in a content schematic is labeled with the average fanout from the source entity to the target entity.
