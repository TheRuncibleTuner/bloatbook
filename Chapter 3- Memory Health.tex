...
\newcommand{\callout}[2]{\vspace{3mm}
\begin{center}
\fcolorbox{black}{Light}{\begin{minipage}{0.85\textwidth}
    \textbf{\large Things To Remember --- #1}
    \hrule
    \vspace{2mm}
...
\callout{The Memory Bloat Factor}{
    An important metric for evaluating the health of a data model
    design is the fraction of overhead in occurences of that data
    model. The \emph{memory bloat factor}, or simly \emph{bloat
      factor}, is this ratio of overhead to total size. In Java, it is
    all too easy to design a data model with a bloat factor well in
    excess of 50\%; this would mean over half of your heap is devoted to things unrelated to the data your application cares about.
}

% TODO: add alignment
\begin{figure}
  \centering
  \includegraphics[width=.50\textwidth]{Figures/eight-char-string.pdf}
...
\item \emph{Pointers}. Both null and non-null pointers glue objects together into data structures.   
...
Every Java string is really two objects: a {\tt String} that serves as a wrapper, and a {\tt char} array with the actual characters. Two objects means two object headers, at 12 bytes each, alignment costs, plus the pointer gluing the two objects together. The {\tt String} object is entirely overhead, containing no real data. Part of its cost is three bookkeeping fields, such as a saved copy of its hashcode computation, that consume a total of 12 bytes. Adding all of this together, the overhead is 48 bytes, which is 75\% of the total space consumed by an eight-character string. The actual data occupies only 25\%. These numbers vary from one JVM to another, but the overhead is always very high.\footnote{All measurements are from the 32-bit IBM Java 6 JVM.}
\end{example}
...
design. The memory health approach looks at entities and collections
separately, since there are different kinds of choices and pitfalls in
each.

\callout{The Entity-Collection (EC) Diagram}{To better understand the
  health of complex data structures, it is useful to have a diagram
  notation that spells out the impacts of your collection choices.
  Think of an EC diagram as a cousin to an Entity-Relation (ER)
  diagram, one that exposes just the right level of implementation
  detail to help you calculate the memory bloat factor of a complex
  data structure.

\begin{center}
  \includegraphics[width=0.7\textwidth]{Figures/ec-diagram-callout}
\end{center}

In an EC diagram, there are two types of boxes, octagons and
rectangles. Each summarizes the implementation details of portion of a
data structure.  Octagons represent collection infrastructure, and
rectangles represent the entities of your application.

It is important to remember that an EC diagram represents the {\em
  containment} structure of your data model. So, an edge from a
collection to a rectangular entity indicates membership of the entity
in the collection. This also means that, if a certain type of
collection or entity is used in multiple ways in the same data
structure, then this type will appear in multiple locations --- one
for each location in the data structure in which that type is used.

The notation $x N = M$ inside each node means there are a total of $N$
instances of that type in that location in the data structure, and in
total these instances occupy $M$ bytes of memory. Each edge in a
content schematic is labeled with the average fanout from the source
entity to the target entity.
\label{callout:ec-diagram}
...
diagram does not include.  An EC diagram summarizes away the
attributes of your entities into a single number: the space occupied
by those attributes. Rather than represent relations, an EC diagram
represents collection choices; where an ER diagram would show a single
relation node for an M-to-N relation, an EC diagram would have a node
for however many collections are necessary to implement this relation
--- typically two.
...
The entities in an EC diagram are abstracted from the actual data
structure objects. A box may be hiding other objects that help
implement the entity. For example, the \texttt{String} entity in
Figure~\ref{fig:content-schematic} represents both \texttt{String} and
\texttt{char} array objects. The name of an entity is the name of the
top object. Entities also summarize objects of the same type that are
repeated in a data structure.  The \texttt{String} entity in
...
Consider the following problem. A monitoring program collects samples from a distributed system. Each sample it collects consists of a timestamp and a value, each represented as a double. The samples arrive out of order. The task is to display samples in chronological order, after all of the data has been collected. The solution requires a data structure to store the samples.  
%\emph{Load-and-Use Behavior Pattern:} load data in one phase, use the data in the next phase.

This section describes the first of three designs for this data structure, which associates a value with each timestamp. For now, assume there are 100 samples. Each design has vastly different memory health characteristics. For each of these designs, the amount of actual data is the same. A double consumes eight bytes, each sample has two doubles, so 100 samples occupies 1.6KB of actual data. Any difference in size among the designs will be in the overhead.

A map is a convenient way to store key-value pairs. A regular \texttt{HashMap} only solves part of the problem. To display the samples in chronological order, the entries have to be sorted. 
...
To understand the costs of this design, it is helpful to construct an Entity-Collection Diagram, or EC diagram, as shown in Figure~\ref{fig:content-schematic-treemap-doubles}.  This diagram gives a simplified, schematic view of the layering of the entities and collections that make up a design, along with their costs. The diagram shows that the total cost of this data structure is 8.5KB.  Since there is only 1.6KB of data, the overhead is 7KB, which is 82\%.  

\begin{figure}
  \centering
  \includegraphics[width=0.4\textwidth]{Figures/content-schematic-treemap-doubles}
  \caption{Solution 1: \texttt{TreeMap} of \texttt{Doubles}}
  \label{fig:content-schematic-treemap-doubles}
\end{figure} 
 
Looking at the individual parts of the EC diagram can provide some insight into the source of this overhead. First, the double values are stored in \texttt{Double} objects. This is because the standard Java collections API operates only on Objects, not scalars, forcing you to box your scalars before putting them in a collection.\footnote{Even with Java's autoboxing, the compiler still generates a boxed scalar in order to store a value in a standard collection.}  A single instance of a \texttt{Double} object is 24 bytes. Since the data is only 8 bytes,  33\% of a \texttt{Double} object is actual data, and 67\% overhead. This is a high price for a basic data type. 

%Move this later? At best, the overhead will be 67\% if the doubles are boxed.

The next surprise is that the \texttt{TreeMap} infrastructure by itself consumes a whopping 3.9KB of memory. All of this is overhead. What is taking up so much space? A \texttt{TreeMap}, like every other collection in Java, has a wrapper object, the \texttt{TreeMap} object itself, along with other internal objects that implement the collection functionality. Each type of collection in the standard library has a different kind of infrastructure, some are built from arrays, some from separate entry objects, and some use a combination of both. --- stopped here --- In the case of \texttt{TreeMap}, for each entry, there is a separate \texttt{Treemap\$Entry} object. This is typical of collections. A typical collection has a fixed overhead, in this case it is 48 bytes, and a per-entry overhead cost, in this case it is 40 bytes per entry. These 40 bytes include five pointer fields, in addition to other sources of overhead. This relatively high per-entry cost makes \texttt{TreeMap} a particularly expensive collection. The costs of other collections are given in Chapter 7.
...
\begin{figure}
  \centering
 \includegraphics[width=0.175\textwidth]{Figures/content-schematic-arraylist-doubles}
...
\begin{figure}
  \centering
  \includegraphics[width=0.4\textwidth]{Figures/content-schematic-arrays-doubles}
...
\begin{figure}
  \centering
  \includegraphics[width=0.5\textwidth]{Figures/scalable-health-treemap}
...
\begin{figure}
  \centering
  \includegraphics[width=0.5\textwidth]{Figures/scalable-health-arraylist}
...
\begin{figure}
  \centering
  \includegraphics[width=0.5\textwidth]{Figures/scalable-health-array}
