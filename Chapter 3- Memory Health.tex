...
A data structure may be big, but how do you know if it is too big? This chapter introduces the concept of memory health, a way to gauge how efficiently a data structure uses memory. Java developers often have to piece together many smaller objects to build a single data structure. Memory health is a way to understand the overhead introduced in each object, the impact of connecting these objects into larger data structures, and, finally, how a data structure scales as it grows. Understanding memory health can provide valuable insight into each step of the data modeling process.

Later chapters show how to estimate both the size and health of data designs. Both of these are important. Memory health is independent of size: a small structure can be unhealthy, and a large structure can be healthy. A healthy data structure will generally scale well, while unhealthy data structures are often the root causes of memory footprint problems. 
...
\callout{callout:memory-bloat-factor}{The Memory Bloat Factor}{
    ...
    design is the fraction of overhead in objects of that data
...
collections are necessary to implement this relation.
...
A map is a convenient way to store key-value pairs. A regular \texttt{HashMap} only solves part of the monitoring system problem. To display the samples in chronological order, the entries have to be sorted. 
...
\section{Two Space-Efficient Designs}
\label{better-designs} 

For the purpose of comparison, this section describes two other designs to represent the monitoring system samples. Like the \texttt{TreeMap} solution, each design associates a value with each timestamp. Note that for 100 samples, there is always 1.6KB of actual data. Any difference in size among different designs will be in the overhead.

The next design uses a more efficient data structure than \texttt{TreeMap}. The sorted order is not maintained when the data is being loaded, but this is not a problem. This design stores the double map in an \texttt{ArrayList}, where each entry is a pair of doubles. There is a bit more programming required to implement this solution, but it is not excessive. Fortunately, the Java standard \texttt{Collections} class has some useful static methods, namely, a \texttt{sort} method and a \texttt{binarySearch} method. Each of these can take an \texttt{ArrayList} and a \texttt{Comparable} as parameters. This design requires writing a new class:
    ...
    public class Pair implements Comparable<Pair> {{
        ...
        public double getValue() {
	      ...
	      public int compareTo(Pair p) {
            return Double.compare(key, p.getKey());	
        }
    }
...
\ttfamily
\begin{verbatim}
    public class DoubleMap implements java.util.Map<Double, Double> {
        ...
        public Double get(Double key) {
            Pair p = new Pair(key.doubleValue(), 0.0);
            int result = Collections.binarySearch(ar, p);
...
The last alternative is an optimal, no-overhead solution. Eliminating overhead requires eliminating objects altogether. This is not a recommended practice, unless memory is extremely tight, and there is no other choice. It is none the less interesting to compare this best case solution with the others. It uses two parallel arrays of doubles. One array stores all of the keys, and the second stores all of the values. You need to encapsulate these arrays in a wrapper object, to provide an object façade.
...
The EC diagram for two arrays of 100 doubles is shown in 
