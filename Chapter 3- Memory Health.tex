...
To help with this task of designing complex, and healthy, models, you
will find it helpful to construct an Entity-Collection (EC) Diagram
for your data model.  \thingstoremember{callout:ec-diagram} presents
the detailed structure of an EC diagram.  It is important to remember
that the entities in an EC diagram are abstracted from the actual data
structure objects. A box may be hiding other objects that help
implement the entity. For example, a \texttt{String} entity in will
represent both the string data type and the underlying character
array.

\callout{callout:ec-diagram}{The Entity-Collection (EC) Diagram}{To
  better understand the health of complex data structures, it is
  useful to have a diagram notation that spells out the impacts of
  your collection choices.  Think of an EC diagram as a cousin to an
  Entity-Relation (ER) diagram, one that exposes just the right level
  of implementation detail to help you calculate the memory bloat
  factor of a complex data structure.
...
rectangles. Each box summarizes the implementation details of portion
of a data structure.  Octagons represent collection infrastructure,
...
--- one for each location in the data structure in which that type is
used.
...
total cost in the single sizing number shown in each node. Where they
would show relations, an EC diagram would have a node for however many
collections are necessary to implement this relation --- typically
two.
...
%\label{section:mapofdoubles}
 ...
 \begin{example}[Constructing an EC Diagram for a Monitoring System]
   A monitoring program collects samples from a distributed
   system. Each sample it collects consists of a timestamp and a
