...
space.
For
some subset of an object's {\em actual} lifetime, your application will make
use of the data stored in its fields.
After the last use of this object's fields, 

Figure~\ref{fig:typical-lifecycle1} illustrates the lifecycle of a typical
object in a well behaved application.

For example, consider this snippet of code, which
...
represented as a string, it will produce a \class{java.util.Date}. In doing so,
objects are created. Some are these are obvious, because the programmer who
wrote this called \code{new} twice. Many more are created by the standard
...
\begin{figure}
	...
	\subtable[When an object exits limbo.]{
		\begin{tabular}{ll} \toprule
        	referenced by  & condition when object exits limbo
        	\\ \cmidrule(r){1-1} \cmidrule(l){2-2}
        	%
        	field of \class{java.lang.ref.SoftReference} & approximately
        	LRU$^{**}$
        	\\
        	%
        	field of \class{java.lang.ref.WeakReference} & upon next garbage
        	collection
        	\\
        	%
        	any other object's field & 
        	\emph{never} --- memory leak
        	\\
        	%
        	local variable & when enclosing method returns
        	\\
        	%
        \bottomrule \addlinespace
        \end{tabular}
	}
	\caption{An object enters a kind of limbo, after its last use.
	Programmers have to be careful that their objects exit limbo in a
	timely fashion. When an object exits limbo is under programmer
	control: depending on how it is referenced, it will be reclaimed
	under certain rules, or may be part of a memory leak. $^{**}$Note: Great care
	is needed when using soft references; see the text for more details.
	%older JREs	use very poor heuristics for handling soft references; see the
...
\begin{figure}
	...
	\label{fig:typical-lifecycle1}
			\includegraphics[width=0.95\textwidth]{Figures/object-lifecycle}
	}
	\subfigure[A situation where there are long periods between uses of an
	object's data.]{
	\label{fig:typical-lifecycle2}
		\includegraphics[width=0.95\textwidth]{Figures/object-lifecycle-lulls}
	}
