...
Your program probably has medium-to-long running methods that implement the
core functionality of the program. For example, if your application includes
loading in a log file from disk, parsing it, and then display the results to
the user, it has roughly three phases for this activity. Most of the objects
allocated in one phase are scoped to that phase; they are needed to implement
the logic of that phase, but not subsequent phases. For example, when loading
the log file, you may maintain some maps that help to cross reference different
parts of the log file. Once the log file has been loaded, these maps can be
discarded. These maps live and die with the first phase of this example
program. If they don't, then you have a memory
