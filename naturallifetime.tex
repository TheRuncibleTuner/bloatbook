...
This task is made simpler by the fact that there aren't innumerable ways in which
objects live and die. For the most part, your objects will fall into one of five
common patterns of lifetime. \autoref{tab:five-lifetimes} summarizes these five
important patterns. Your application needs some objects to live forever and it
needs the rest to die a timely death. 
%This often requires careful design on your
%part, both to avoid bugs and so that your application performs well in the case
%when not everything fits into the Java heap. 
The trickier aspects of memory
management, summarized in \autoref{tab:tricky-memory-management}, are discussed
