...
The next chapter goes into greater detail of how to avoid these kinds of
...
The fifth main facet of object lifetime are those objects that stay around
beyond the scope of any one method or object. These objects must survive for
some indeterminant amount of time; their lifetime is not correlated with any
easily identifiable entity in the program. In some cases, this period based on
the profitability of keeping them around. In other cases, objects need to be
kept around to support multiple independent operations across several threads.
...
\subsection{Caches and User/Connection State}

If the data stored in an object is cheap to recompute or refetch from an
external data source, then a good policy would be to make sure that the object
lives only as long as it is clearly needed. Performing a few dozen machine instructions is very
likely to be a worthwhile trade-off, if memory consumption is the limiting
resource for scaling up\index{Scaling Up}. If, on the other hand, this data is
very expensive to recompute or refetch, and there is some chance it might be
used in the future, then it might be worthwhile to keep it around. 

Deferring
the time that an object will be reclaimed is the primary function of a cache. A cache is a performance optimization 
that trades space for time\index{Time-Space Trade-offs}.
It holds on to a data structure
after the current operation is finished with it, in the hope that other operations in the
