...
In limbo, an object will never be used again, or at least not for long time, but
the JRE doesn't yet know that this is the case. The object hangs
...
The point at which an object exits its limbo depends upon how it is referenced
by other objects.

\begin{figure}
	\centering
	\subfigure[After its last use, an object enters a kind of limbo.]{
		\includegraphics[width=\textwidth]{Figures/lifetime/states}
	}
	\subtable[Rules for exiting limbo. $^{**}$Note: Care is needed when using soft
	references.]{ \begin{tabular}{ll} \toprule
	...
	\caption{The point when an object exits limbo depends on 
