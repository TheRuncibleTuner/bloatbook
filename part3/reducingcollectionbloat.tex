...
This is the end of the optimizations on small collections.
The final entity-collection diagram for the graph example is shown in
...
68\%, which is quite significant. The bloat factor has gone from 95.7\% to
86.6\%, and you might wonder why the bloat factor is still so high? The answer
is that the overhead of nesting an \class{ArrayList} in a \class{HashMap} is
still very high compared to the data, which is just two \class{Integers}. 
The next chapter talks more about high per-entry costs,
and various techniques to reduce the cost of nested collections.
\begin{figure}
  \centering
...
This chapter describes a number of ways to mitigate high memory costs from too
