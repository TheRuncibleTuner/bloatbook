...
action, element arrays are almost always too big.
 ...
 Figure~\ref{fig:trimmed-graph}. The total size is reduced by 25.6\%, and the
 bloat factor goes from 94.6\% to 92.7\%. This is still rather high, but there
 are more optimizations ahead.
...
Too many empty collections filling the heap is another common problem. A quick
look inside an empty collection shows that it is not all that empty. 
According to Table~\ref{tab:collection-costs}, an empty \class{HashMap} consumes
...
diagram after removing 25,000 empty edge \class{ArrayLists}.
\begin{figure}
  \centering
...
\end{figure}
 ...
 Delaying allocation avoids creating too many empty collections.
...
of the story. Another problem that is easily fixed is the implementation of
\class{Edge} is overly-general. In particular, we know that the
\class{ArrayList} in \class{Edge} will never be bigger than two entries, so 
it is not necessary to use an expandable array --- a simple array will do.
...
each edge. This simple change eliminates 9.16 megabytes, as shown in 
Figure~\ref{fig:arraylist-to-array}.
\begin{figure}
  \centering
