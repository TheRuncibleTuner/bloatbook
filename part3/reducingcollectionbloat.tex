...
memory it uses. At one end of the spectrum, a simple \class{ArrayList} is the
most memory efficient, and at the other end, something like a
\class{ConcurrentHashMap} offers sophisticated concurrent access
...
\class{HashMap}, where the keys are nodes and the values are \class{HashSet}s of edges. 
 Figure~\ref{fig:graph-hashset} shows an entity-collection diagram for a
 graph.
 \begin{figure}
  \centering
...
%
% Edge 16*400,000 = 6.4 million = 

There are several bloat problems here. The first problem is that 
 100,000 very small \class{HashSets} each consume 232 bytes,
 all overhead. It's hard to think of a good reason why such a heavy-weight
 structure like \class{HashSet} should ever be used for just a handful of entries.
 For small sets, \class{ArrayList} is almost always a better choice.
\class{HashSet} maintains uniqueness and provides fast access, but uniqueness
can be enforced by linear scanning without significant performance loss. 
Figure~\ref{fig:graph-arraylist} 
shows improved memory usage with \class{ArrayList}.
...
This example shows the importance of comparing costs of different collection
choices. It's instructive to look more closely at an example of a collection,
to understand how memory is being used. \class{HashSet} is particularly
interesting, being both typical and bloated. 


 Figure~\ref{fig:hashset} shows why a \class{HashSet}
...
there is never a need for a set of values.
 ...
 Additionally, each \class{HashMap\$Entry} stores a hashcode, which is an
 unnecessary redundant field. The most common keys are either
 \class{Strings}, which store their own hashcode, or \class{Integers}, whose
 hashcodes are easy to compute. 

Show the cost of all collections: include per entry cost.
How to calculate: fixed  objects (used in any collection) + 
expandable costs (array size + per
entry costs)

 Show the cost of a HashSet (includes HashMap.
More generally, the cost includes fixed objects + expandable objects;
See table in figure ?? for 
See figure ?? that shows each of these in terms of their fixed costs and
expandable costs.
L
