...
This small-collections pattern is a leading cause of memory bloat.
Collections themselves are pure overhead, since they are infrastructure glue, 
not data, and they are quite big. Our guess is that the
collection class developers would be surprised by this usage pattern. 
Why bother worrying about expandable structures and clever
hashing algorithms for only a few entries? 

 This chapter describes the cost of the most common collection classes, and
 shows how to reduce the bloat from having too many small
 collections.
 
 \section{Choosing The Right Collection}

In this case 
the planning system was implementing what is know as a level graph. A level 
graph is a graph with nodes and the graph has multiple levels, where each node 
has different edges at each level. So same nodes at each level, different edges 
