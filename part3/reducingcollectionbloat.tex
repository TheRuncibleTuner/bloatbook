...
arbitrarily big, but, as we have seen, this functionality comes at a cost.
Collections include wrapper objects, and those collections that have entry
arrays are sized with extra growth room. If you know that the size
of a collection is fixed, the the ability to expand a collection is an
unnecessary function, and some of the overhead can be eliminated. Often, it is sufficient to use a
simple Java array, and not use a collection at all.  

Let's look more closely at an \class{Edge} in the graph example, shown in
Figure~\ref{fig:edges}(a). An \class{Edge} object is a wrapper containing a
two-element \class{ArrayList}. The \class{ArrayList} object is itself another
wrapper pointing to an \class{Integer} array. 
Since an \class{Edge} will never store more than two \class{Integers}, it is 
not necessary to store these in an \class{Arraylist} --- a simple array will do.
...
\begin{figure}
  \centering
 \includegraphics[width=.80\textwidth]{part3/Figures/Edges.pdf}
...
This is an example of choosing a collection which is overly-general. In
...
transformed a dynamic representation of a type, where the fields may not be known at compile-time, to
...
These transformations are the final optimizations on the graph example.
The resulting entity-collection diagram is shown in
Figure~\ref{fig:arraylist-to-array}. The total size from the first
entity-collection diagram in Figure~\ref{fig:graph-hashset} has been reduced by
68\%, and the bloat factor has gone from 95.7\% to 86.6\%. The size reduction
is quite significant, but why is the bloat factor still so high? The answer is
that the cost of the per entry overhead of each value in the \class{HashMap} is
still very high compared to the cost of two \class{Integers}. This cost is
harder to eliminate in Java. The next chapter talks more about per-entry costs,
and various techniques to reduce bloat.
\begin{figure}
  \centering
