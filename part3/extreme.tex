...
If you need the convenience of an actual \class{Node} object, you can have a bit
...
transient node objects act as facades to the \class{INodes} model, but require a
reference to the \class{NodeModel} and the node's index.
...
\begin{figure}
\centering
\subfigure[Each node has an index.]{
...
At this point, it is
easy to establish an index, as an extra attribute of the \class{INodes} model.
