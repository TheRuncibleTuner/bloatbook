...
\callout{ezscalability}{Deriving Scalability Formulas \textsc{[For Experts]}}{
...
is $C/A$. We want to solve for the highest bloat factor, $x$, that is just small
enough to let you fit at least one element of data in memory.
The total unit cost of your data, including overhead is
$\frac{1}{1-x} D$. Thus:
\begin{align*}
...
A common implementation strategy, applied to the \class{Node} and \class{Edge}
...
\class{Node} data type. Following this strategy, the \class{Node} class has
three fields, to store its \class{Color} and the relations to children and
