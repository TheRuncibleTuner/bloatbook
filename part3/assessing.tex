...
database row is an entity. Columns such as numbers, dates, string, and binary
large objects (BLOBs) columns are all attributes of these entities. The
cross-table index data, such as the employees under a manager, are handled
differently, at least in a database. In a database, this information is usually
stored in a separate table; e.g. a table that maps managers to the employees
they supervise. In Java, this isn't a very natural way to model things. It is
much more convenient to model relationships as references to collections of
other entities: a manager object points to a set of employee objects. The Java
heap itself is a graph of objects, and so this is a natural, though possibly
inefficient, mapping of relational data into Java classes and fields.
% \begin{example}{Storing a Graph}

If a graph has a great many nodes and edges, you must design its storage
carefully. Consider the small example illustrated in \autoref{fig:exampleGraph}.
There are several ways to implement the abstract data types, of nodes and edges,
shown in that figure. Each strategy has its positives and negatives, depending
on whether ease of maintenance or memory consumption are of primary importance.
% \end{example}
