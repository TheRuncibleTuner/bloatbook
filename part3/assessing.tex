...
\begin{itemize}
  ...
  within a given memory budget. This requires knowing the \emph{variable cost}
  of each user.
...
the \autoref{part:1}. This requires knowing how how bloat factor changes as a
...
\callout{datastructurescaling}{Unit Costs and Data Structure Scaling}{ Every
data structure has a \emph{unit cost}, the cost for every additional element. This
unit cost governs how big it will get, and consequently how many elements fit
into a given budget.

A data structure \emph{scales well}, or not, depending upon how its bloat factor
changes as its size increases. Up till now, bloat factor has been a scalar
quantity: e.g., 95\% of space is implementation overhead. More accurately, the
bloat factor of a data structure is a function of its size. Its bloat factor
will decrease, as fixed costs are amortized over a larger amount of actual data.
}
