...
\class{Edge} objects per node. This increases the effective cost per node to
...
interface: two calls to the \code{parents()} interface no longer have
...
Implementations} If the API method \code{Node.children()} returns a set of
