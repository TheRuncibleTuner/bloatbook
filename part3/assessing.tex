...
\textbf{\textrm{asymptotic bloat factor}} &= \frac{V + \frac{B}{1-B} U}{V +
\frac{1}{1-B} U}
\end{align*}

For example, the variable cost of a \class{HashMap} is 48 bytes. If you're
storing structures with a unit cost of 40 bytes each with a bloat
factor of 50\%, then the asymptotic bloat factor is $\frac{48 + 0.5/(1-.5) *
40}{48 + 1/(1-.5)*40} = 69\%$. No matter how many elements you store in this
data structure, you will achieve no better than 31\% actual data.
...
The formula behind this chart depends upon a
couple of straightforward quantities: let $M$ be the bytes of memory available,
$N$ be the number of entities, and $D$ be the unit cost of your actual data, and
$x$ be the unknown asymptotic bloat factor that you need to solve for.
...
depends on the (unknown) asymptotic bloat factor $x$ and $D$. The
total unit cost of your data, including overhead is $\frac{1}{1-x} D$; e.g. 20
bytes of actual data with a bloat factor of 60\% means the total size per entity
is 50 bytes (because $50 - 50 * 0.6 = 20$).  Thus:
% \begin{align*}
$$\frac{M}{N} \frac{1}{\frac{1}{1-x} D}  \ge 1
 %\\
% \frac{M (1-x)}{N D}  &\ge 1
% \\
% 1-x  &\ge \frac{N D}{M}
...
interface INode {
   Color getColor();
   Set<IEdge> getChildren();
   Set<IEdge> getParents();
}
interface IEdge {
   INode getFrom();
   INode getTo();
...
\texttt{node.getChildren().get(5).getTo()\-.getColor()} and, later, quickly
...
depends on the asymptotic bloat factor, as things scale up. 
...
This means the asymptotic bloat factor of every node:
\begin{align*}
...
\frac{1}{1-B} U} = 96\%
\end{align*}
That's a very high degree of bloat, and seems to merit some tuning.


Using \class{ArrayList} instead of a hash map would lower the total unit cost
per node from 404 bytes down to $B$ factor to 224 bytes, 
%header + two 40-byte ArrayList plus 10*4*2 (default array size is 10)
resulting in an asymptotic bloat factor of 93\%. This is still high, but this
small change has increased the number of nodes you can fit in a gigabyte from
2.8 million up to 4.8 million. 

That's great, but, 93\% bloat factor is still bad. So there's still room for big
improvements. If you know that the number of parents and children will be
typically no more than two, then you can use a better initial size for the edge
lists. If you update the constructor call for the \class{ArrayList}s to request
an initial capacity of two elements, then the total unit cost of the nodes drops
to 160 bytes. This only drops the asymptotic bloat factor to 90\%, but increases
your capacity for nodes to 6.7 million.
